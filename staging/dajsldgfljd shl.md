# IFRS ESG 에이전트 — 기준서 PDF 벡터 DB 저장과 DART API 통합

ESG 공시 자동화를 위한 IFRS 에이전트에서, 기준서 PDF를 벡터 DB에 저장하는 부분과 DART 전자공시 API를 크롤링하는 부분을 정리했습니다. 두 기능 모두 구현 가능하며, 현재 상태와 구현 방법을 차례로 다룹니다.

## 1. 벡터 DB에 기준서 PDF 저장

LlamaParse 또는 Unstructured로 PDF를 파싱한 뒤 벡터 DB에 저장하는 방식입니다.

### 현재 상태
- `generate_embeddings.py`: DB 테이블(DataPoint 등)의 임베딩 생성
- PDF → 벡터 DB 저장 전용 스크립트는 아직 없음
...
class DARTClient:
    ...
    def download_report(self, rcept_no: str) -> Optional[bytes]:
        ...
        # 실제로는 보고서 다운로드 API 사용 또는 웹 크롤링으로 PDF 다운로드
        return None

    def get_sustainability_reports(self, corp_code: str, year: int) -> List[Dict]:
        """지속가능경영보고서 목록 조회"""
        bgn_de = f"{year}0101"
        end_de = f"{year}1231"
        ...
        sustainability_reports = [
            r for r in reports
            if "지속가능" in r.get("report_nm", "")
            or "ESG" in r.get("report_nm", "")
            or "sustainability" in r.get("report_nm", "").lower()
        ]
...
async def _crawl_external_sources(
    ...
    for report in reports:
        ...
        if pdf_content:
            parsed = await self._parse_pdf(pdf_content)  # PDF 파싱 (LlamaParse)
            documents.append({
                "content": parsed,
...
1. **벡터 DB 저장 스크립트 작성** — PDF 파싱(LlamaParse/Unstructured) → 청크 분할 → 임베딩 생성 → PostgreSQL + pgvector 저장
2. **DART API 클라이언트 구현** — 기업 코드 조회 → 보고서 목록 조회 → 보고서 다운로드 → RAG Node 통합
3. **통합 테스트** — IFRS S2 기준서를 벡터 DB에 저장 → DART API로 삼성SDS 보고서 크롤링 → RAG Node에서 검색 테스트

실행 예시는 다음과 같습니다.

```bash
python -m ifrs_agent.main --generate "IFRS S2 기후 관련 배출량 데이터 추출" \
    --standards IFRS_S2 --fiscal-year 2024 --company-id samsung-sds

python -m ifrs_agent.main --test-rag --generate "IFRS S2 기후 관련 배출량 데이터 추출" \
    --standards IFRS_S2 --fiscal-year 2024 --company-id samsung-sds --target-dps S2-15-a S2-16-a
```

## 4. 데이터베이스 테이블 — `data_points`

ESG 지표의 최소 공시 단위를 저장하는 핵심 테이블입니다.

| 컬럼명 | 타입 | 설명 | 예시 |
| --- | --- | --- | --- |
| **dp_id** | String(50) | Data Point 고유 식별자 (PK) | `"S2-29-a"`, `"GRI-305-1"` |
...
| **validation_rules** | JSONB | 검증 규칙 | `{"min": 0, "max": 1000000, "required": true}` |
| **value_range** | JSONB | 값 범위 | `{"min": 0, "max": 1000000}` |
...
| **embedding_text** | Text | 임베딩 생성에 사용된 텍스트 | `"Scope 1 온실가스 배출량 ..."` |
| **embedding_updated_at** | TIMESTAMP | 임베딩 업데이트 시각 | `2024-01-01 12:00:00` |

## 5. RAG 검색 파이프라인 흐름

전체 검색·추출 흐름은 6단계로 구성됩니다.
...
if company_id & fiscal_year 존재 AND (회사 데이터 부족 OR 외부 데이터 필요):
    └─> _crawl_external_sources(company_id, fiscal_year)
        ...
        ├─> _get_sustainability_reports(corp_code, fiscal_year)
        └─> 각 보고서 처리:
            ├─> _download_report(rcept_no)           # PDF 다운로드
            ├─> DocumentService.store_pdf_to_vector_db()
            └─> 텍스트 추출 (PyMuPDF)
...
state["yearly_data"] = _organize_by_year(fact_sheets)
